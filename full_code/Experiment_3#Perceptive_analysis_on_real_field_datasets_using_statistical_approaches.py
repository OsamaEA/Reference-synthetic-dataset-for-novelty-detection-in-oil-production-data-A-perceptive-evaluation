...
# the code was originallry run using median_absolute_deviation from scipy 1.5.0 which is obselete now
...
outliers = pd.read_excel('.\inputs\wells_100_outliers_fractions.xlsx')
...
# # 2. Processing
def stats_criteria(technique, scores):
    '''
        Function to selecet the statistica technique to use for threshold cutoff
        INPUT: 
              technique: string of the statistical method to use
              scores: original scores calculated by each algorithm
        OUTPUT:
              predictions list of outliers
                                                                                    '''
    # Thresholding              
    scores_df = pd.DataFrame(scores)
    # Scaling scores dataset for equal weights for each algorithm
    scores_scaled_df = pd.DataFrame(MinMaxScaler().fit_transform(scores_df))
        
    if technique == 'IQR':
        # IQR Approach
        # Setting upper and lower quantiles
        q1 = scores_scaled_df.quantile(0.25)
        q3 = scores_scaled_df.quantile(0.75)
        # calculating IQR
        iqr = q3 - q1
        ...
        # STD Approach
        # Average value and standdard deviation
        meann = scores_scaled_df.mean()[0]
        stdd = scores_scaled_df.std()[0]
        # Setting boundary for outliers
        upper = meann + alpha * stdd        

    else:
        # MAD Approach
        # setting median value
        med = scores_scaled_df.median()[0]
        # calculating mad range
        mad = median_abs_deviation(scores_scaled_df)[0]
        # setting upper boundaries
        upper = med + alpha * mad
        
    # predictions of outliers and inliers
    preds = pd.DataFrame(np.where((scores_scaled_df > upper), 1, 0)).iloc[:,0].tolist()
        
    return(preds)

def outliers_plotting(technique, alpha, df_plot_full_cols, df_trial, model, perc, ground_truth, original_days, original_outliers_indices_days, technique_name, evaluation_matrix, x_col_plot, y_col_plot):
    '''
        INPUT:
    ...
    if type(model) == list:
        ...
        # using a pre-defined function to interpret predictions
        preds = stats_criteria(technique, scores)
    ...
    Precision, Recall, f_beta_score, _ = precision_recall_fscore_support(ground_truth, preds, average = 'binary')
...
def compare_OD_methods(technique, alpha, df_original, outliers_df_original, columns_to_include = 'all', x_col_plot = 'days', y_col_plot = 'Qo_bpd', scaling = True):
    '''
        INPUT:
    ...
    for i, well_name in enumerate(df.columns.get_level_values(0).unique().tolist()):
        ...
        # arbitary value for filling purposes only
        perc = 0.00001
        ...
        COMBO_matrix = outliers_plotting(technique, alpha, df_plot_full_cols, df_trial, 
        ...
        ABOD_matrix = outliers_plotting(technique, alpha, df_plot_full_cols, df_trial, Abod_clf, perc, ground_truth, original_days, original_outliers_indices_days, 'ABOD', ABOD_matrix, x_col_plot, y_col_plot)

        '''initiatory'''
        # uncomment the below line for visualization simulatneously
        #ax5 = fig.add_subplot(2, 5, 5, sharex = ax, sharey = ax)
        SOS_clf = SOS(contamination = perc, perplexity = 12)
        SOS_matrix = outliers_plotting(technique, alpha, df_plot_full_cols, df_trial, SOS_clf, perc, ground_truth, original_days, original_outliers_indices_days, 'SOS', SOS_matrix, x_col_plot, y_col_plot)
        ...
        KNN_matrix_avg = outliers_plotting(technique, alpha, df_plot_full_cols, df_trial, KNN_clf, perc, ground_truth, original_days, original_outliers_indices_days, 'KNN avg', KNN_matrix_avg, x_col_plot, y_col_plot)
        ...
        LOF_matrix = outliers_plotting(technique, alpha, df_plot_full_cols, df_trial, LOF_clf, perc, ground_truth, original_days, original_outliers_indices_days, 'LOF', LOF_matrix, x_col_plot, y_col_plot)
        ...
        IFor_matrix = outliers_plotting(technique, alpha, df_plot_full_cols, df_trial, IForest_clf, perc, ground_truth, original_days, original_outliers_indices_days, 'iForest', IFor_matrix, x_col_plot, y_col_plot)

        # uncomment the below line for visualization simulatneously
        #ax9 = fig.add_subplot(2, 5, 9, sharex = ax, sharey = ax)
        COF_clf = COF(contamination = perc, n_neighbors = 7)
        COF_matrix = outliers_plotting(technique, alpha, df_plot_full_cols, df_trial, COF_clf, perc, ground_truth, original_days, original_outliers_indices_days, 'COF', COF_matrix, x_col_plot, y_col_plot)
        
        # uncomment the below line for visualization simulatneously
        #ax10 = fig.add_subplot(2, 5, 10, sharex = ax, sharey = ax)
        KNN_median_clf = KNN(contamination = perc, n_neighbors=4, method = 'median')
        KNN_median_matrix = outliers_plotting(technique, alpha, df_plot_full_cols, df_trial, KNN_median_clf, perc, ground_truth, original_days, original_outliers_indices_days, 'KNN', KNN_median_matrix, x_col_plot, y_col_plot)
...
##### Results on real-field dataset using MAD, STD, and IQR on 100 wells!

df_stat_all_techniques = []
for stat_technique in ['MAD', 'STD', 'IQR']:
    # Looping through values of alpha
    for alpha in np.arange(0.1,3.1,0.1):
        # Ex
        df_stat = compare_OD_methods(stat_technique, alpha, df, outliers, ['days', 'Qo_bpd'])
        df_stat.columns = pd.MultiIndex.from_product([['{} alpha'.format(alpha)], df_stat.columns])
        df_stat=pd.concat([df_stat],keys=[stat_technique],axis=1)

        df_stat_all_techniques.append(df_stat)
    print("Successfuuly Extracted Results of Algorithms Performance using {} on Field Dataset!".format(stat_technique))
df_statistics = pd.concat(df_stat_all_techniques, axis = 1)
df_statistics.to_csv('.\outputs\experiment_3\Field_data_stats_performance.csv')

# # 4. Results Visualization

# #### A) Loading original dataset as was run using Scipy v 1.5.0!

df_outlying_stats = pd.read_csv('.\inputs\Field_data_stats_performance.csv', header = [0,1,2], index_col = [0])
df_mad_all = df_outlying_stats['MAD']
df_std_all = df_outlying_stats['STD']
df_iqr_all = df_outlying_stats['IQR']
perc_list = np.arange(0.1,3.1, 0.1)

# #### B) Performance of all models using different statistical techniques of MAD, STD, and IQR!

# Statistical techniques
stats_techniques = df_outlying_stats.columns.get_level_values(0).unique().tolist()
# Evaluation metrics for models evaluation on the bootstrapped dataset
evaluation_metrics = ['cohen_kappa', 'matthews_phi_coef', 'f2_score', 'Accuracy',
                      'Recall', 'Precision','Fallout', 'auc_score', 'maP','execution_time']
for stat in stats_techniques:
    df_single_stats = df_outlying_stats[stat]
    fig = plt.figure(figsize = (25, 25))
    for i, matrix in enumerate(evaluation_metrics):
        # Adding subplots in a loop
        ax = plt.subplot(4,3,i+1)
        # Characteristics of each plot
        plt.plot(perc_list, df_single_stats.loc[:, df_single_stats.columns.get_level_values(1) == matrix].loc['COMBO'].tolist(), '--', c = 'lightcoral', label = 'Ensemble')
        plt.plot(perc_list, df_single_stats.loc[:, df_single_stats.columns.get_level_values(1) == matrix].loc['ABOD'].tolist(), 'o', c = 'green', label = 'ABOD', alpha = 0.5)
        plt.plot(perc_list, df_single_stats.loc[:, df_single_stats.columns.get_level_values(1) == matrix].loc['SOS'].tolist(), 's', c = 'blue', label = 'SOS', alpha = 0.5)
        plt.plot(perc_list, df_single_stats.loc[:, df_single_stats.columns.get_level_values(1) == matrix].loc['KNN_median'].tolist(), 'D', c = 'magenta', label = 'KNN Median', alpha = 0.5)
        plt.plot(perc_list, df_single_stats.loc[:, df_single_stats.columns.get_level_values(1) == matrix].loc['COF'].tolist(), '^', c = 'red', label = 'COF', alpha = 0.5)
        plt.plot(perc_list, df_single_stats.loc[:, df_single_stats.columns.get_level_values(1) == matrix].loc['KNN'].tolist(), '>', c = 'orange', label = 'KNN', alpha = 0.5)
        plt.plot(perc_list, df_single_stats.loc[:, df_single_stats.columns.get_level_values(1) == matrix].loc['iForest'].tolist(), '<', c = 'black', label = 'iForest', alpha = 0.5)
        plt.plot(perc_list, df_single_stats.loc[:, df_single_stats.columns.get_level_values(1) == matrix].loc['LOF'].tolist(), 'v', c = 'lightseagreen', label = 'LOF', alpha = 0.5)
        plt.yticks(np.arange(0, 1.1, 0.1));
        plt.xlabel('{} alpha fraction'.format(stat));
        plt.ylabel(matrix + ' score');
        plt.title(matrix);

    # Title for the entire graph
    plt.suptitle('Performance of different Models using {} statistical technique'.format(stat), fontsize = 20, y = 0.93);
    # Mutual Legend
    handles, labels = ax.get_legend_handles_labels();
    fig.legend(handles, labels, loc='right', handlelength=1, handleheight = 7, fontsize = 12);
    # Saving figure
    plt.savefig('.\outputs\experiment_3\Statistical_performance_on_100_randomly_generated_datasets_using_{}.png'.format(stat))


# #### C) Visualization of performance of various statistical techniques on Ensemble model!
# Statistical techniques
stats_techniques = df_outlying_stats.columns.get_level_values(0).unique().tolist()
# Evaluation metrics for models evaluation on the bootstrapped dataset
evaluation_metrics = ['cohen_kappa', 'matthews_phi_coef', 'f2_score', 'Accuracy',
...
colors_dict = {'MAD' : 'r', 'STD' : 'b', 'IQR' : 'g'}
fig = plt.figure(figsize = (25, 25))
for i, matrix in enumerate(evaluation_metrics):
    df_single_matrix = df_outlying_stats.iloc[:, df_outlying_stats.columns.get_level_values(2) == matrix]
    ax = plt.subplot(4,3,i+1)
    for stat in stats_techniques:
        df_single_stats = df_single_matrix[stat]
        # Characteristics of each plot
        ax.plot(perc_list, df_single_stats.loc['COMBO'].tolist(), '--', c = colors_dict[stat], label = 'Ensemble - {}'.format(stat))
        plt.yticks(np.arange(0, 1.1, 0.1));
        plt.xlabel('alpha fraction');
        plt.ylabel(matrix + ' score');
        plt.title(matrix);
        plt.legend()

# Title for the entire graph
plt.suptitle('Comparison of different statistical techniques of Ensemble model', fontsize = 20, y = 0.93);
# Saving figure
plt.savefig('.\outputs\experiment_3\Comparison_of_different_statistical_techniques_of_ensemble_model.png'.format(stat))


# #### D) Performance of Ensemble-MAD against all algorithms using different statistical techniques!
# Statistical techniques
stats_techniques = df_outlying_stats.columns.get_level_values(0).unique().tolist()
# Evaluation metrics for models evaluation on the bootstrapped dataset
evaluation_metrics = ['cohen_kappa', 'matthews_phi_coef', 'f2_score', 'Accuracy']
fig = plt.figure(figsize = (25, 25))
for j, matrix in enumerate(evaluation_metrics):
    df_single_matrix = df_outlying_stats.iloc[:, df_outlying_stats.columns.get_level_values(2) == matrix]
    for i, stat in enumerate(stats_techniques):
        ax = plt.subplot(4,3,(i+1) + j*3)
        df_single_stats = df_single_matrix[stat]
        # Characteristics of each plot
        plt.plot(perc_list, df_single_matrix['MAD'].loc['COMBO'].tolist(), '--', c = 'lightcoral', label = 'Ensemble - MAD')
        plt.plot(perc_list, df_single_stats.loc[:, df_single_stats.columns.get_level_values(1) == matrix].loc['ABOD'].tolist(), 'o', c = 'green', label = 'ABOD', alpha = 0.5)
        plt.plot(perc_list, df_single_stats.loc[:, df_single_stats.columns.get_level_values(1) == matrix].loc['SOS'].tolist(), 's', c = 'blue', label = 'SOS', alpha = 0.5)
        plt.plot(perc_list, df_single_stats.loc[:, df_single_stats.columns.get_level_values(1) == matrix].loc['KNN_median'].tolist(), 'D', c = 'magenta', label = 'KNN Median', alpha = 0.5)
        plt.plot(perc_list, df_single_stats.loc[:, df_single_stats.columns.get_level_values(1) == matrix].loc['COF'].tolist(), '^', c = 'red', label = 'COF', alpha = 0.5)
...
# Title for the entire graph
plt.suptitle('Performance of different Models using {} statistical technique against Ensemble-MAD technique'.format(stat), fontsize = 20, y = 0.93);
# Mutual Legend
handles, labels = ax.get_legend_handles_labels();
fig.legend(handles, labels, loc='right', handlelength=1, handleheight = 7, fontsize = 12);
# Saving figure
plt.savefig('.\outputs\experiment_3\Comparison_of_different_statistical_techniques_of_algorithms_against_Ensemble_MAD.png')
